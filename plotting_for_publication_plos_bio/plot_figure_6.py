...
import sys
import numpy
from utils import diversity_utils, core_gene_utils, sample_utils, stats_utils
...
from numpy.random import randint, random, choice, shuffle
...
for species_name in good_species_list:
    ...
    from plos_bio_scripts import calculate_snp_prevalences, calculate_private_snvs, calculate_temporal_changes, \
...
for sample_pair in replacement_map['hmp'].keys():
    print sample_pair, len(replacement_map['hmp'][sample_pair]), replacement_map['hmp'][sample_pair]
...
for cohort in cohorts:
    ...
    if cohort=='hmp':
        ...
        # Random between host
        xs, ns =       stats_utils.calculate_unnormalized_survival_from_vector(pooled_between_snp_change_distribution[cohort], min_x=1e-02, max_x=1e09)
    ...
    print "Observed dNdS", observed_dNdSs
    print "Lower CI", bootstrapped_dNdSs[long(0.025*num_bootstraps),:]
    print "Upper CI",bootstrapped_dNdSs[long(0.975*num_bootstraps),:]
    ...
    for i in xrange(1,len(total_freq_all_snps[cohort])/2+1):
        ...
        print derived_freq_bins[i], non_counts, syn_counts, observed_dNdS, bootstrapped_dNdSs[long(0.025*num_bootstraps)], bootstrapped_dNdSs[long(0.975*num_bootstraps)]
