...
genomicData_tmp=read.table(args[4],sep=",")
genomicData=genomicData_tmp[,1:(sampleSize+1)]
annoFile_tmp=read.table(args[5],sep=",")
annoFile=annoFile_tmp[,1:(sampleSize+1)]
outFile = args[6]
gene_name = args[7]
...
StrainOrder<-strsplit(as.character(analysisWindow[1,4]),",")[[1]]
NumStrainsCluster<-as.numeric(strsplit(as.character(analysisWindow[1,3]),",")[[1]])
...
# purple = syn pol
# orange = ns pol
colorChart=c('white','blue','red','skyblue','orange')
#colorChart=c('white','blue','red','white','white')
...
#make empty plot
	...
	title(main=paste(gene_name),cex.main = 1.8)	
...
#start with most common haplotype at bottom
	...
	for (i in StrainClusterOrder){	
		...
		for (j in 1:windowSize){
			rect(j-1,height[1],j,height[2],density=-1,col=(ZeroOneHaplotypes[i,j]),lwd=0., border=NA)	
		}
		h=h+1} 
