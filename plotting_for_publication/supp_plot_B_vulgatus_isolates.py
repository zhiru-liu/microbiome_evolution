...
def preprocess_data(dat):
    within_ct, between_ct, full_df = close_pair_utils.merge_and_filter_transfers(
        dat, separate_clade=True, merge_threshold=0, filter_threshold=0, ignore_pairs=False)
    ...
    between_full_lengths = np.array(
        [pair_to_total_length.get(x, 0) for x in dat['pairs']]) * config.second_pass_block_size
    ...
    mask = (clonal_fractions > config.clonal_fraction_cutoff)  # & (clonal_divs < clonal_div_cutoff)

    return total_ct, within_ct, between_ct, full_df, clonal_divs, mask
...
def plot_between_within_length_distributions(ax, full_df):
    ...
    conversion_factor = 7.17  # 4D to full genome length conversion; found in isolates jupyter notebook
    within_lens = within_lens * config.second_pass_block_size * conversion_factor
    between_lens = between_lens * config.second_pass_block_size * conversion_factor
    ...
    _ = ax.hist(within_lens, cumulative=-1,
                histtype='step', bins=100, density=True, label='Within-clade')
    _ = ax.hist(between_lens, cumulative=-1,
...
def plot_between_within_clade(ax, est_Ts, within_counts, between_counts, plot_trend=True):
    core_genome_len = 1497264
...
path = os.path.join(config.analysis_directory, "closely_related", "isolates", "MGYG-HGUT-02478.pickle")
genome_len = 208000
data = pickle.load(open(path, 'rb'), encoding='latin1')

total_counts, within_counts, between_counts, full_df, clonal_divs, mask = preprocess_data(data)

transfer_df_path = os.path.join(config.analysis_directory, "closely_related", 'isolates',
                  'MGYG-HGUT-02478_all_transfers.pickle')
full_df = pd.read_pickle(transfer_df_path)

full_df = full_df[full_df['clonal fraction'] >= config.clonal_fraction_cutoff]
print("Total number of close pairs: %d, detected transfers: %d, within transfers: %d, between transfers: %d" % (len(clonal_divs[mask]), full_df.shape[0], np.sum(full_df['types'] == 0), np.sum(full_df['types'] == 1)))

plot_between_within_clade(between_within_count_ax, clonal_divs[mask], within_counts[mask], between_counts[mask], plot_trend=False)
