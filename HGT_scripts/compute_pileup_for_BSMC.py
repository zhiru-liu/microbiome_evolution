...
# generic script for computing pileup for BSMC simulation
if __name__ == "__main__":
    # data_dir = os.path.join(config.analysis_directory, 'fastsimbac_data', 'for_pileup', 'b_vulgatus')
    data_dir = os.path.join(config.analysis_directory, 'fastsimbac_data', 'hotspot')
    # ckpt_dir = os.path.join(config.analysis_directory, 'sharing_pileup', 'simulated', 'b_vulgatus')
    ckpt_dir = os.path.join(config.analysis_directory, 'sharing_pileup', 'simulated', 'hotspot')
    DEBUG=False
    t = 0.00825
    genome_len = 280000
