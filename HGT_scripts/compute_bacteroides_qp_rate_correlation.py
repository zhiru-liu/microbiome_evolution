...
axes[1].plot(bacteroides_df['QP frac'], bacteroides_df['mean rates'], '.')
axes[1].set_ylim([None, 100000])
axes[1].set_yscale('log')
axes[1].set_xlabel('Fraction of QP samples')
axes[1].set_ylabel('Mean Rates')
print("Correlation using only the mean is:")
print(spearmanr(bacteroides_df['QP frac'], bacteroides_df['mean rates']))

plt.savefig(os.path.join(config.figure_directory, 'supp', 'qp_frac_rate_corr.pdf'), bbox_inches='tight')
