...
import plos_bio_scripts.calculate_linkage_disequilibria

def plot_species(species_name):
    fig, example_axis = plt.subplots(figsize=(4, 3))

    figdir = os.path.join(config.analysis_directory, "LD")
    ld_map = plos_bio_scripts.calculate_linkage_disequilibria.load_ld_map(species_name, cf=0.0)
    cf_ld_map = plos_bio_scripts.calculate_linkage_disequilibria.load_ld_map(species_name, cf=0.1)
